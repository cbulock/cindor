...
import { EmbTreeItem } from "../tree-item/emb-tree-item.js";

/**
 * Keyboard-accessible single-selection tree view.
 *
 * Use nested `emb-tree-item` elements to create branches and leaves.
 *
 * @fires input - Fired when the selected item changes.
 * @fires change - Fired when the selected item changes.
 *
 * @slot - Direct child `emb-tree-item` elements.
 */
export class EmbTreeView extends LitElement {
  static styles = css`
    :host {
      display: block;
      color: var(--fg);
  ...
  private handleTreeItemSelect = (event: Event): void => {
    const detail = (event as CustomEvent<{ item?: EmbTreeItem }>).detail;
    if (detail.item) {
  ...
  private handleKeyDown = (event: KeyboardEvent): void => {
    const currentItem = event.composedPath().find((node): node is EmbTreeItem => node instanceof EmbTreeItem);
  ...
  private handleExpandKey(item: EmbTreeItem): void {
  ...
  private handleCollapseKey(item: EmbTreeItem): void {
    if (this.childItems(item).length > 0 && item.expanded) {
      item.toggle(false);
      this.refreshTree();
      return;
  ...
  private selectAndFocus(item: EmbTreeItem): void {
  ...
  private selectItem(item: EmbTreeItem): void {
  ...
  private parentItem(item: EmbTreeItem): EmbTreeItem | null {
    let current: HTMLElement | null = item.parentElement;

    while (current) {
      if (current instanceof EmbTreeItem) {
        return current;
      }
  ...
  private levelFor(item: EmbTreeItem): number {
    let level = 1;
  ...
  private collectVisibleItems(root: ParentNode = this): EmbTreeItem[] {
    const items = Array.from(root.children).filter((child): child is EmbTreeItem => child instanceof EmbTreeItem);
    const result: EmbTreeItem[] = [];
  ...
  private get allItems(): EmbTreeItem[] {
    return Array.from(this.querySelectorAll("emb-tree-item")).filter((child): child is EmbTreeItem => child instanceof EmbTreeItem);
  }

  private get visibleItems(): EmbTreeItem[] {
  ...
  private childItems(item: EmbTreeItem): EmbTreeItem[] {
    return Array.from(item.children).filter((child): child is EmbTreeItem => child instanceof EmbTreeItem);
  }

  private itemButton(item: EmbTreeItem): HTMLButtonElement | null {
    return item.shadowRoot?.querySelector('[part="item"]') as HTMLButtonElement | null;
  }

  private itemValue(item: EmbTreeItem): string {
