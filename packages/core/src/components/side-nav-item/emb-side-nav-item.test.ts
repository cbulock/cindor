...
import { EmbSideNavItem } from "./emb-side-nav-item.js";

describe("emb-side-nav-item", () => {
  it("renders links with current-page semantics", async () => {
    const element = document.createElement("emb-side-nav-item") as EmbSideNavItem;
    element.href = "/docs";
    element.label = "Docs";
  ...
  it("toggles child visibility for grouped items", async () => {
    ...
    element.innerHTML = '<emb-side-nav-item label="Getting started"></emb-side-nav-item>';
    document.body.append(element);
    await element.updateComplete;
