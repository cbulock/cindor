 ...
 *
 * @slot - Direct child `emb-splitter-panel` elements.
 */
export class EmbSplitter extends LitElement {
  static styles = css`
    :host {
      display: block;
      min-width: 0;
  ...
  private panels: EmbSplitterPanel[] = [];
  ...
  private syncPanels(): void {
    this.panels = Array.from(this.children).filter((child): child is EmbSplitterPanel => child instanceof EmbSplitterPanel);
    this.setSizes(
