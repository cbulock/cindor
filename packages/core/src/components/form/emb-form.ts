...
export class EmbForm extends LitElement {
  static styles = css`
    :host {
      display: grid;
      gap: var(--emb-form-gap, var(--space-4));
      width: var(--emb-form-inline-size, min(100%, 48rem));
      max-width: 100%;
      min-width: 0;
      color: var(--fg);
    }
    ...
    ::slotted(form) {
      display: grid;
      gap: var(--emb-form-content-gap, var(--space-4));
      width: 100%;
      max-width: 100%;
      min-width: 0;
    }
  ...
  private getManagedFields(): FormFieldElement[] {
    ...
    return Array.from(this.currentForm.querySelectorAll("emb-form-field")) as FormFieldElement[];
  }

  private getOwningField(control: ValidatableElement): FormFieldElement | null {
    return control.closest("emb-form-field") as FormFieldElement | null;
  }
