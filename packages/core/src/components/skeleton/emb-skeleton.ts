...
export class EmbSkeleton extends LitElement {
  static styles = css`
    :host {
      display: block;
      inline-size: var(--emb-skeleton-width, 100%);
      max-inline-size: 100%;
    }
    ...
    span {
      display: block;
      inline-size: 100%;
      block-size: var(--emb-skeleton-height, 1rem);
      ...
      animation: emb-skeleton-pulse 1.2s ease-in-out infinite;
    }
    ...
    :host([variant="block"]) span {
      block-size: var(--emb-skeleton-height, 4rem);
    }

    :host([variant="circle"]) span {
      inline-size: 100%;
      block-size: var(--emb-skeleton-size, 2.5rem);
      border-radius: 50%;
    }

    :host([variant="circle"]) {
      inline-size: var(--emb-skeleton-size, 2.5rem);
    }

    @keyframes emb-skeleton-pulse {
      0% {
