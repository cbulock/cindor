...
import { EmbCalendar } from "../calendar/emb-calendar.js";
import { LitElement } from "lit";

/**
 * Popup date range picker powered by the Emberline range calendar.
 *
 ...
 */
export class EmbDateRangePicker extends LitElement {
  static styles = css`
    :host {
      display: inline-block;
      width: var(--emb-field-inline-size, min(100%, 360px));
      max-width: 100%;
      min-width: 0;
      color: var(--fg);
    }
      ...
      <div class="surface">
        <div class="field" part="field" tabindex="0" @click=${this.handleFieldClick} @keydown=${this.handleFieldKeyDown}>
          ...
          ${this.startValue || this.endValue
            ? html`
                <emb-icon-button label="Clear range" name="x" part="clear-button" @click=${this.handleClear}></emb-icon-button>
              `
            : null}
          <emb-icon-button
  ...
  private handleCalendarInput = (event: Event): void => {
    event.stopPropagation();
    const calendar = event.currentTarget as EmbCalendar;
  ...
  private get panelElement(): HTMLElement | null {
    return this.renderRoot.querySelector("emb-calendar");
  }
