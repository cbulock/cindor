...
export class EmbCommandPalette extends LitElement {
  static styles = css`
    :host {
      display: contents;
    }
    ...
    return html`
      ...
      >
        <div class="surface" @keydown=${this.handleKeyDown}>
          ...
          ${commands.length
            ? html`
                <emb-listbox active-index=${String(this.activeIndex)} part="listbox">
                  ${commands.map(
                    (command) => html`
                      <emb-option
  ...
  private get searchElement(): HTMLElement | null {
    return this.renderRoot.querySelector("emb-search");
  }
}
