...
- If verified pushes fail in an older clone because of a stale local pre-push hook, run `npm run hooks:repair` once from the repo root. Fresh `npm install` runs the same cleanup automatically via `prepare`.
